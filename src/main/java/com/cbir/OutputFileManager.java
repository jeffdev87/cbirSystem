...
public class OutputFileManager {
	...
	public Vector<Distance> calcDistancias(double vetCaracRef[], String imgNameRef, DistanceType optDist) {
		...
		try {
			while (this.input.hasNextLine()) {
				String line = this.input.nextLine(), imgNameBase = line.substring(0, line.indexOf(":"));
				...
				if (!imgNameRef.equals(imgNameBase)) {
					...
					double vetCarac[] = DistanceFunctions.parseSringToDouble(vetString, vetString.length);

					Distance estDist = new Distance(imgNameRef, imgNameBase);
					switch (optDist) {
					case EUCLIDIAN:
						estDist.dist = DistanceFunctions.distEuclidiana(vetCaracRef, vetCarac, vetString.length);
						break;
					case MANHATAN:
						estDist.dist = DistanceFunctions.distManhatan(vetCaracRef, vetCarac, vetString.length);
						break;
